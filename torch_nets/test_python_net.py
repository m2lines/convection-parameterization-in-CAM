...
model2 = torch.jit.load("torchscript_model.pt")  # load from the torchscript weights
model3 = load_from_netcdf_params(
...
actual3 = model3.forward(x).detach().numpy()

assert np.all((actual1 == actual2) & (actual2 == actual3))
